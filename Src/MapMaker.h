...
protected:
  ...
  boost::atomic<bool> mbResetRequested;   // A reset has been requested
  boost::atomic<bool> mbResetDone;        // The reset was done.
  boost::atomic<bool> mbBundleAbortRequested;      // We should stop bundle adjustment
  boost::atomic<bool> mbBundleRunning;             // Bundle adjustment is running
  boost::atomic<bool> mbBundleRunningIsRecent;     //    ... and it's a local bundle adjustment.
};
